...
# -------------------------------
def permutation_importance_manual(model, X_val, y_val, feature_names, n_repeats=5):
    """Compute permutation importance manually for a Keras model."""
    baseline = mean_squared_error(y_val, np.maximum(0, model.predict(X_val).flatten()), squared=False)
    importances = {}
...
def run_pipeline():
    ...
    lr = callbacks.ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=3, min_lr=1e-6, verbose=1)
    ...
    # --- Feature importance
    importances_df = permutation_importance_manual(model, X_test, y_test, feature_cols, n_repeats=5)
